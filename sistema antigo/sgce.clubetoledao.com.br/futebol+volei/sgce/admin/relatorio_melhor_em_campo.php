...
// 2. Busca os MVPs de todas as partidas do campeonato, agrupados por jogador e equipe
$sql_mvps = "
    SELECT 
        ...
        eq.id as id_equipe,
        COUNT(p.id_melhor_jogador) as total_mvp
    FROM partidas p
    ...
    ORDER BY total_mvp DESC, par.nome_completo ASC
";
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Melhores em Campo</li>
    </ol>
</nav>

<main class="container py-5">
    <div class="card">
        <div class="card-header"><i class="fas fa-crown text-success me-2"></i>Melhores em Campo do Campeonato</div>
        <div class="card-body">
            <?php if (count($mvps) > 0): ?>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr class="caption-event <?= $row_class ?>">
                                    ...
                                    <td><?= htmlspecialchars($mvp['total_mvp']) ?></td>
                                </tr>
                                <tr class="body-event d-none">
                                    <td colspan="3">
                                        <?php
                                        // Busca as partidas onde este jogador foi MVP
                                        $sql_partidas = "
                                            SELECT DISTINCT
                                                p.id as id_partida,
                                                ...
                                                (COALESCE(p.placar_equipe_a, 0) + COALESCE(p.placar_equipe_b, 0)) as total_gols_partida
                                            FROM partidas p
                                        ...
                                        ?>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <strong>Placar: </strong><?= htmlspecialchars($partida['placar_equipe_a'] ?? '-') ?> x <?= htmlspecialchars($partida['placar_equipe_b'] ?? '-') ?><br>
                                                <strong>Total de Gols: </strong><?= htmlspecialchars($partida['total_gols_partida']) ?>
                                            </div>
                                            <!-- Tabela de gols -->
                                            <table class="table inner-table">
                                                <?php
                                                // Busca os gols detalhados para a partida
                                                $sql_gols = "
                                                    SELECT 
                                                    ...
                                                    WHERE se.id_partida = ? AND se.tipo_evento = 'Gol'
                                                    GROUP BY par.id, eq.id
                                                    ORDER BY CASE WHEN eq.id = ? THEN 1 ELSE 0 END DESC, total_gols DESC, par.nome_completo ASC
                                                ";
                                                $stmt_gols = $pdo->prepare($sql_gols);
                                                $stmt_gols->execute([$partida['id_partida'], $partida['id_equipe_a']]);
                                                $gols = $stmt_gols->fetchAll(PDO::FETCH_ASSOC);

                                                $gols_equipe_a = array_filter($gols, fn($gol) => $gol['id_equipe'] == $partida['id_equipe_a']);
                                                $gols_equipe_b = array_filter($gols, fn($gol) => $gol['id_equipe'] == $partida['id_equipe_b']);
                                                ?>
                                                <!-- Equipe A -->
                                                <?php if (!empty($gols_equipe_a)): ?>
                                                    <thead>
                                                        <tr>
                                                            <th colspan="4"><div class="equipe-a-header w-100">Gols de <?= htmlspecialchars($partida['nome_equipe_a']) ?></div></th>
                                                        </tr>
                                                        <tr>
                                                            <th>Número</th>
                                                            <th>Jogador</th>
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($gols_equipe_a as $gol): ?>
                                                            <tr>
                                                                <td><?= htmlspecialchars($gol['numero'] ?? '-') ?></td>
                                                                <td><?= htmlspecialchars($gol['nome_participante']) ?></td>
                                                                <td><?= htmlspecialchars($gol['apelido'] ?? '-') ?></td>
                                                                <td><?= htmlspecialchars($gol['total_gols']) ?></td>
                                                            </tr>
                                                        <?php endforeach; ?>
                                                    </tbody>
                                                <?php endif; ?>
                                                <!-- Equipe B -->
                                                <?php if (!empty($gols_equipe_b)): ?>
                                                    <thead>
                                                        <tr>
                                                            <th colspan="4"><div class="equipe-b-header w-100">Gols de <?= htmlspecialchars($partida['nome_equipe_b']) ?></div></th>
                                                        </tr>
                                                        <tr>
                                                            <th>Número</th>
                                                            <th>Jogador</th>
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($gols_equipe_b as $gol): ?>
                                                            <tr>
                                                                <td><?= htmlspecialchars($gol['numero'] ?? '-') ?></td>
                                                                <td><?= htmlspecialchars($gol['nome_participante']) ?></td>
                                                                <td><?= htmlspecialchars($gol['apelido'] ?? '-') ?></td>
                                            ...
                                            <?php if (empty($gols_equipe_a) && empty($gols_equipe_b)): ?>
                                                <div class="alert alert-info text-center">Nenhum gol registrado para esta partida.</div>
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                                        <?php else: ?>
                                            <div class="alert alert-info text-center">Nenhuma partida registrada para este jogador como MVP.</div>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <?php else: ?>
                <div class="alert alert-secondary text-center">Nenhum MVP foi registrado para este campeonato.</div>
            <?php endif; ?>
        </div>
    </div>
</main>

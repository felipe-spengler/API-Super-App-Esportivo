...
if ($_SERVER['REQUEST_METHOD'] !== 'GET' || !isset($_GET['id_campeonato'])) {
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div>
            <button type="button" class="btn btn-success btn-sm me-2" id="btnAdicionarRodada">
                <i class="fas fa-plus me-2"></i>Adicionar Rodada
            </button>
            ...
            <div class="tab-content" id="rodadasContent">
                <div class="tab-pane active" id="rodada1">
                    <div class="row g-3 partidas-container" data-rodada="1ª Rodada">
                        <?php if (!$necessita_bye): ?>
                            <?php $global_index = 0; ?>
                            <?php foreach ($partidas_propostas as $partida): ?>
                                <div class="col-md-6 col-lg-4">
                                    <div class="card h-100 shadow-sm">
                                        <div class="card-body d-flex justify-content-around align-items-center">
                                            <input type="hidden" name="partidas[<?= $global_index ?>][equipe_a]" value="<?= $partida['id_a'] ?>">
                                            <input type="hidden" name="partidas[<?= $global_index ?>][equipe_b]" value="<?= $partida['id_b'] ?>">
                                            ...
                                            <input type="hidden" name="partidas[<?= $global_index ?>][rodada]" value="<?= htmlspecialchars($partida['rodada']) ?>">
                                        ...
                                        <div class="card-footer d-flex justify-content-center">
                                            <button type="button" class="btn btn-sm btn-editar-partida position-absolute top-0 end-0 px-0 me-4" 
                                                    data-partida='<?= json_encode($partida) ?>' 
                                                    title="Editar Confronto">
                                                <i class="fas fa-pen"></i>
                                            </button>
                                            <button type="button" class="btn btn-sm btn-excluir-partida position-absolute top-0 end-0 me-0" 
                                                    data-id-partida="<?= $global_index ?>" 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add new rodada
    document.getElementById('btnAdicionarRodada').addEventListener('click', () => {
        rodadaCount++;
        const newTabLi = document.createElement('li');
        newTabLi.className = 'nav-item';
        newTabLi.innerHTML = `<a class="nav-link" href="#rodada${rodadaCount}" data-bs-toggle="tab">${rodadaCount}ª Rodada</a>`;
        rodadasTabs.appendChild(newTabLi);

        const newTabPane = document.createElement('div');
        newTabPane.className = 'tab-pane';
        newTabPane.id = `rodada${rodadaCount}`;
        newTabPane.innerHTML = `<div class="row g-3 partidas-container" data-rodada="${rodadaCount}ª Rodada"></div>`;
    ...
    btnSalvarEdit.addEventListener('click', () => {
        ...
        if (hiddenEquipeA && hiddenEquipeB && hiddenData && hiddenLocal && hiddenFase && hiddenRodada) {
    ...
    if (selectBye) {
        selectBye.addEventListener('change', () => {
            ...
            // Hide placeholder
            if (placeholderPartidas) placeholderPartidas.style.display = 'none';

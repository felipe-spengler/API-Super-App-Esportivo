...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['registrar_tempo'])) {
    ...
    try {
        ...
        if ($acao === 'iniciar') {
            ...
            $stmt_period = $pdo->prepare("INSERT INTO sumulas_periodos (id_partida, periodo, hora_inicio, id_equipe_a, id_equipe_b) VALUES (?, ?, ?, ?, ?)");
            $stmt_period->execute([$id_partida, $periodo, $hora_inicio, $id_equipe_a, $id_equipe_b]);
            $response = ['success' => true, 'type' => 'success', 'message' => "Partida e $periodo iniciados com sucesso!"];
            $stmt = $pdo->prepare("SELECT hora_inicio FROM partidas WHERE id = ?");
            $stmt->execute([$id_partida]);
            $response['hora_inicio'] = $stmt->fetchColumn();
        } elseif ($acao === 'finalizar') {
            $id_equipe_timeout = $_POST['id_equipe_timeout'] ?? null;
            $stmt_current = $pdo->prepare("SELECT id, periodo FROM sumulas_periodos WHERE id_partida = ? AND hora_fim IS NULL AND periodo NOT LIKE 'Pausa %'" . ($id_equipe_timeout ? " AND (id_equipe_a = ? OR id_equipe_b = ?)" : "") . " LIMIT 1");
            $params = [$id_partida];
            if ($id_equipe_timeout) {
                $params[] = $id_equipe_timeout;
                $params[] = $id_equipe_timeout;
            }
            ...
            if ($open_period) {
                ...
                if ($open_period['periodo'] === 'Penalti') {
                    $response = ['success' => true, 'type' => 'success', 'message' => 'Pênalti finalizado com sucesso!'];
                } else {
                    ...
                    $stmt->execute([$id_partida]);
                    $stmt = $pdo->prepare("SELECT hora_fim FROM partidas WHERE id = ?");
                    $stmt->execute([$id_partida]);
                    $response['hora_fim'] = $stmt->fetchColumn();
                }
            } else {
                $response['message'] = 'Partida já finalizada ou não iniciada.';
            }
        } elseif ($acao === 'timeout') {
            $periodo = $_POST['periodo_timeout'] ?? '';
            if (!$periodo) {
...
// --- BUSCA DE DADOS ---
try {
    $stmt_jogadores = $pdo->prepare("SELECT id, nome_completo, id_equipe FROM participantes WHERE id_equipe IN (?, ?) ORDER BY nome_completo");
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="ver_partidas.php?id_campeonato=<?= htmlspecialchars($partida['id_campeonato']) ?>">Partidas</a></li>
        ...
        <form id="time-control-form" method="POST">
            ...
            <div class="row g-3 align-items-end">
                <div class="col-md-3">
                    ...
                    <button type="submit" class="btn btn-success w-100" id="start-btn"><i class="fas fa-play me-1"></i> Iniciar</button>
                </div>
                <?php if ($current_period): ?>
                <div class="col-md-3">
                    <button type="button" id="finalizar-periodo-btn" class="btn btn-secondary w-100"><i class="fas fa-stop me-1"></i> Finalizar Período Atual (<?= htmlspecialchars($current_period) ?>)</button>
                </div>
                <?php endif; ?>
                <div class="col-md-3">
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form id="pause-control-form" method="POST">
            <input type="hidden" name="registrar_tempo" value="1">
            <div class="row g-3 align-items-end">
                <input type="hidden" id="periodo_pausa" name="periodo_timeout" class="form-control" value="<?= $curent_periodo_pause ?>" readonly>
                <div class="col-md-3" style="<?php if($current_period!='Penalti' ||$current_period+="Pausa Penalti"){?>display:none<?php }?>">
                    <label for="id_equipe_pausa" class="form-label">Equipe</label>
                    <select name="id_equipe_timeout" id="id_equipe_pausa" class="form-select" >
                        <option value="" disabled selected>Selecione...</option>
                        <option value="<?= htmlspecialchars($partida['id_equipe_a']) ?>"><?= htmlspecialchars($partida['nome_equipe_a']) ?></option>
                        <option value="<?= htmlspecialchars($partida['id_equipe_b']) ?>"><?= htmlspecialchars($partida['nome_equipe_b']) ?></option>
                    </select>
                ...
                <div class="col-md-3">
                    <button type="submit" name="acao" value="iniciar" class="btn btn-warning w-100" id="start-pause-btn" <?=  $partida['hora_fim'] ? 'disabled' : '' ?>>
                ...
                <tr>
                    ...
                    <td id="pausas_penalti_equipe_a">
                        ...
                        echo implode(', ', array_map(function($t) use ($partida) {
                            ...
                            $str = date('H:i:s', $start_time);
                            $str .= $t['hora_fim'] ? ' - ' . date('H:i:s', $end_time) : ' - Em andamento';
                            $str .= " ($duration min, {$partida['nome_equipe_a']})";
                            return $str;
                ...
                <tr>
                    ...
                    <td id="pausas_penalti_equipe_b">
                        ...
                        echo implode(', ', array_map(function($t) use ($partida) {
                            ...
                            $str = date('H:i:s', $start_time);
                            $str .= $t['hora_fim'] ? ' - ' . date('H:i:s', $end_time) : ' - Em andamento';
                            $str .= " ($duration min, {$partida['nome_equipe_b']})";
                            return $str;
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    <?php if (empty($eventos)): ?>
                        <tr><td colspan="6" class="text-center text-muted">Nenhum evento registrado.</td></tr>
                    <?php else: ?>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Manipular botão de finalizar período
    if (finalizarPeriodoBtn) {
        finalizarPeriodoBtn.addEventListener('click', () => {
            sendTimeControlRequest(timeForm, 'finalizar_periodo');
        });
    }
    ...
    let periodsData = [
        ...
        <?php if (strpos($p['periodo'], 'Pausa ') !== 0): ?>
        {
            ...
            end: <?= $p['hora_fim'] ? 'new Date("' . $p['hora_fim'] . '").getTime()' : 'null' ?>,
    ...
    let pauses = [
        <?php foreach ($timeouts as $t): ?>
        {
            ...
            end: <?= $t['hora_fim'] ? 'new Date("' . $t['hora_fim'] . '").getTime()' : 'null' ?>,
    ...
    let horaFimMatch = <?= $partida['hora_fim'] ? 'new Date("' . $partida['hora_fim'] . '").getTime()' : 'null' ?>;

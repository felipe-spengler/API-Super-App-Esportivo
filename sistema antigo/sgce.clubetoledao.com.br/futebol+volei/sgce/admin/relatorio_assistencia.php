...
$sql_assistencias = "
    ...
    WHERE p.id_campeonato = ? AND se.tipo_evento = 'Assistência'
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Assistências</li>
    </ol>
</nav>

<main class="container py-5">
    <div class="card">
        <div class="card-header"><i class="fas fa-hands-helping text-primary me-2"></i>Assistências do Campeonato</div>
        <div class="card-body">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr class="caption-event <?= $row_class ?>">
                                    ...
                                    <td><?= htmlspecialchars($assistencia['total_assistencias']) ?></td>
                                </tr>
                                <tr class="body-event d-none">
                                    <td colspan="3">
                                        <?php
                                        // Busca as partidas onde este jogador registrou assistências
                                        $sql_partidas = "
                                            SELECT DISTINCT
                                                p.id as id_partida,
                                        ...
                                        ?>
                                            ...
                                            <table class="table inner-table">
                                                <?php
                                                // Busca as assistências detalhadas para a partida
                                                $sql_assistencias_partida = "
                                                    ...
                                                    GROUP BY par.id, eq.id
                                                    ORDER BY CASE WHEN eq.id = ? THEN 1 ELSE 0 END DESC, total_assistencias DESC, par.nome_completo ASC
                                                ";
                                                $stmt_assistencias_partida = $pdo->prepare($sql_assistencias_partida);
                                                $stmt_assistencias_partida->execute([$partida['id_partida'], $partida['id_equipe_a']]);
                                                $assistencias_partida = $stmt_assistencias_partida->fetchAll(PDO::FETCH_ASSOC);

                                                $assistencias_equipe_a = array_filter($assistencias_partida, fn($assistencia) => $assistencia['id_equipe'] == $partida['id_equipe_a']);
                                                $assistencias_equipe_b = array_filter($assistencias_partida, fn($assistencia) => $assistencia['id_equipe'] == $partida['id_equipe_b']);
                                                ?>
                                                <!-- Equipe A -->
                                                <?php if (!empty($assistencias_equipe_a)): ?>
                                                    <thead>
                                                        <tr>
                                                            <th colspan="4"><div class="equipe-a-header w-100">Assistências de <?= htmlspecialchars($partida['nome_equipe_a']) ?></div></th>
                                                        </tr>
                                                        <tr>
                                                            <th>Número</th>
                                                            <th>Jogador</th>
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($assistencias_equipe_a as $assistencia): ?>
                                                            <tr>
                                                                <td><?= htmlspecialchars($assistencia['numero'] ?? '-') ?></td>
                                                                <td><?= htmlspecialchars($assistencia['nome_participante']) ?></td>
                                                                <td><?= htmlspecialchars($assistencia['apelido'] ?? '-') ?></td>
                                                                <td><?= htmlspecialchars($assistencia['total_assistencias']) ?></td>
                                                            </tr>
                                                        <?php endforeach; ?>
                                                    </tbody>
                                                <?php endif; ?>
                                                <!-- Equipe B -->
                                                <?php if (!empty($assistencias_equipe_b)): ?>
                                                    <thead>
                                                        <tr>
                                                            <th colspan="4"><div class="equipe-b-header w-100">Assistências de <?= htmlspecialchars($partida['nome_equipe_b']) ?></div></th>
                                                        </tr>
                                                        <tr>
                                                            <th>Número</th>
                                                            <th>Jogador</th>
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($assistencias_equipe_b as $assistencia): ?>
                                                            <tr>
                                                                <td><?= htmlspecialchars($assistencia['numero'] ?? '-') ?></td>
                                                                <td><?= htmlspecialchars($assistencia['nome_participante']) ?></td>
                                                                <td><?= htmlspecialchars($assistencia['apelido'] ?? '-') ?></td>
                                            ...
                                            <?php if (empty($assistencias_equipe_a) && empty($assistencias_equipe_b)): ?>
                                                <div class="alert alert-info text-center">Nenhuma assistência registrada para esta partida.</div>
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                                        <?php else: ?>

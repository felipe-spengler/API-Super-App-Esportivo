...
$id_partida = isset($input['id_partida']) ? filter_var($input['id_partida'], FILTER_VALIDATE_INT) : null;
...
if (!$id_partida || !$id_jogador || !$categoria || !isset($categoria_to_column[$categoria])) {
...
try {
    $stmt = $pdo->prepare("UPDATE partidas SET $id_column = ?, $foto_column = ? WHERE id = ?");
    $stmt->execute([$id_jogador, $id_foto_selecionada, $id_partida]);

    if ($stmt->rowCount() > 0) {
        echo json_encode(['success' => true]);
    } else {
        echo json_encode(['success' => false, 'message' => 'Nenhuma alteração foi feita na partida.']);
    }
} catch (PDOException $e) {
    error_log("Erro ao salvar jogador: " . $e->getMessage());
    echo json_encode(['success' => false, 'message' => 'Erro ao salvar jogador na partida: ' . $e->getMessage()]);
}
?>
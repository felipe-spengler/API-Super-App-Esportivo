...
// Check for query errors or missing match
if ($stmt_partida->errorCode() !== '00000') {
    $errorInfo = $stmt_partida->errorInfo();
    die("Erro na consulta da partida: " . $errorInfo[2]);
}
if (!$partida) {
    die("Partida não encontrada.");
}
if (!isset($partida['id_equipe_a']) || !isset($partida['id_equipe_b'])) {
    die("Erro: Dados da partida incompletos (id_equipe_a ou id_equipe_b ausente).");
}
...
// Check for query errors or empty participants
if ($stmt_participantes->errorCode() !== '00000') {
    $errorInfo = $stmt_participantes->errorInfo();
    die("Erro na consulta de participantes: " . $errorInfo[2]);
}
if (empty($participantes)) {
    die("Nenhum participante encontrado para as equipes {$partida['id_equipe_a']} e {$partida['id_equipe_b']}.");
}

// Separate players by team (compatible with PHP < 7.4)
$jogadores_a = array_filter($participantes, function($p) use ($partida) {
    return $p['id_equipe'] == $partida['id_equipe_a'];
});
$jogadores_b = array_filter($participantes, function($p) use ($partida) {
    return $p['id_equipe'] == $partida['id_equipe_b'];
});

// Debug: Verify players
if (empty($jogadores_a) && empty($jogadores_b)) {
    die("Erro: Nenhum jogador encontrado para as equipes.");
}
...
if ($stmt_eventos->errorCode() !== '00000') {
    $errorInfo = $stmt_eventos->errorInfo();
    die("Erro na consulta de eventos: " . $errorInfo[2]);
}

// Organize events (compatible with PHP < 7.4)
$gols_a = array_filter($eventos, function($e) use ($partida) {
    return $e['tipo_evento'] == 'Gol' && $e['id_equipe'] == $partida['id_equipe_a'];
});
$gols_b = array_filter($eventos, function($e) use ($partida) {
    return $e['tipo_evento'] == 'Gol' && $e['id_equipe'] == $partida['id_equipe_b'];
});
$cartoes_a = array_filter($eventos, function($e) use ($partida) {
    return in_array($e['tipo_evento'], ['Cartão Amarelo', 'Cartão Azul', 'Cartão Vermelho']) && $e['id_equipe'] == $partida['id_equipe_a'];
});
$cartoes_b = array_filter($eventos, function($e) use ($partida) {
    return in_array($e['tipo_evento'], ['Cartão Amarelo', 'Cartão Azul', 'Cartão Vermelho']) && $e['id_equipe'] == $partida['id_equipe_b'];
});
$substituicoes_a = array_filter($eventos, function($e) use ($partida) {
    return $e['tipo_evento'] == 'Substituição' && $e['id_equipe'] == $partida['id_equipe_a'];
});
$substituicoes_b = array_filter($eventos, function($e) use ($partida) {
    return $e['tipo_evento'] == 'Substituição' && $e['id_equipe'] == $partida['id_equipe_b'];
});
$faltas_a = array_filter($eventos, function($e) use ($partida) {
    return $e['tipo_evento'] == 'Falta' && $e['id_equipe'] == $partida['id_equipe_a'];
});
$faltas_b = array_filter($eventos, function($e) use ($partida) {
    return $e['tipo_evento'] == 'Falta' && $e['id_equipe'] == $partida['id_equipe_b'];
});
...
$esporte = $stmt_esporte->fetch(PDO::FETCH_ASSOC);

// Check for query errors or missing sport
if ($stmt_esporte->errorCode() !== '00000') {
    $errorInfo = $stmt_esporte->errorInfo();
    die("Erro na consulta de esporte: " . $errorInfo[2]);
}
if (!$esporte) {
    die("Esporte não encontrado para id {$partida['id_esporte']}.");
}
...
<body>
    ...
    <table class="table table-bordered table-sm no-border">
        <tr class="no-border">
            ...
            <td style="width: 25%;">
                <b>Rodada:</b> <?= htmlspecialchars($partida['rodada'] ?? 'Não informada') ?>
            </td>
        </tr>
    </table>
    ...
    <div style="position: relative">
        <div style="display: inline-block; width: 75%; margin-right: 2px;">
            ...
            <table style="margin-bottom: 16px" class="table table-bordered table-sm">
                <tr>
                    <td>
                        <b>Arbitragem:</b> <span contenteditable="true"><?= htmlspecialchars($partida['arbitragem'] ?? '') ?></span>
                    </td>
                </tr>
            </table>
        </div>
        <div style="display: inline-block; width: 24%;">
            <table style="margin-bottom: 16px" class="table table-bordered table-sm">
                ...
                <tr>
                    ...
                    <td contenteditable="true"><?= htmlspecialchars($partida['periodo1_inicio'] ?? '') ?></td>
                    <td contenteditable="true"><?= htmlspecialchars($partida['periodo1_fim'] ?? '') ?></td>
                </tr>
                <tr>
                    <td class="table-active"><b>Período 2</b></td>
                    <td contenteditable="true"><?= htmlspecialchars($partida['periodo2_inicio'] ?? '') ?></td>
                    <td contenteditable="true"><?= htmlspecialchars($partida['periodo2_fim'] ?? '') ?></td>
                </tr>
                <tr>
                    <td class="table-active"><b>Período Extra</b></td>
                    <td contenteditable="true"><?= htmlspecialchars($partida['periodo_extra_inicio'] ?? '') ?></td>
                    <td contenteditable="true"><?= htmlspecialchars($partida['periodo_extra_fim'] ?? '') ?></td>
                </tr>
            </table>
        </div>
    </div>
    ...
    <div style="margin-bottom: 4px; position: relative; display: block;">
        <div style="display: inline-block; width: 69%; margin-right: 2px;">
            <table style="font-size: 13pt;" class="table-sm table table-bordered">
                <tbody>
                    <tr>
                        <td colspan="7"><b><?= htmlspecialchars($partida['nome_equipe_a']) ?></b></td>
                    </tr>
                    <tr>
                    ...
                    <?php $i = 1; foreach ($jogadores_a as $jogador): ?>
                        <tr>
                            ...
                            <td style="text-align: center; padding-top: 0px; padding-bottom: 0px; color: lightgrey; font-size: 8pt; white-space: nowrap;">
                                <?php
                                $falta_count = count(array_filter($faltas_a, function($f) use ($jogador) {
                                    return $f['id_participante'] == $jogador['id'];
                                }));
                            ...
                            <td style="text-align: center; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px;">
                                <?php echo array_filter($cartoes_a, function($c) use ($jogador) {
                                    return $c['id_participante'] == $jogador['id'] && $c['tipo_evento'] == 'Cartão Amarelo';
                                }) ? '<span class="font-menor">X</span>' : '<span class="font-menor"> | </span>'; ?>
                            </td>
                            <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;">
                                <?php echo array_filter($cartoes_a, function($c) use ($jogador) {
                                    return $c['id_participante'] == $jogador['id'] && $c['tipo_evento'] == 'Cartão Azul';
                                }) ? '<span class="font-menor">X</span>' : ''; ?>
                            </td>
                            <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;">
                                <?php echo array_filter($cartoes_a, function($c) use ($jogador) {
                                    return $c['id_participante'] == $jogador['id'] && $c['tipo_evento'] == 'Cartão Vermelho';
                                }) ? '<span class="font-menor">X</span>' : ''; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    ...
                    <?php endfor; ?>
                    <tr>
                        <td style="padding-top: 0px; padding-bottom: 0px;" colspan="3"><b>Técnico:</b> <span contenteditable="true"><?= htmlspecialchars($partida['tecnico_a'] ?? '') ?></span></td>
                        <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;"></td>
                        <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;"></td>
                        <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;"></td>
                        <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;"></td>
    ...
    <div style="position: relative; display: block;">
        <div style="display: inline-block; width: 69%; margin-right: 2px;">
            <table style="font-size: 13pt;" class="table-sm table table-bordered">
                <tbody>
                    <tr>
                        <td colspan="7"><b><?= htmlspecialchars($partida['nome_equipe_b']) ?></b></td>
                    </tr>
                    <tr>
                        <td class="table-active" colspan="2" style="width: 70%;"><b>Jogadores</b></td>
                        <td class="table-active" style="text-align: center;"><b>Nº</b></td>
                    ...
                    <?php $i = 1; foreach ($jogadores_b as $jogador): ?>
                        <tr>
                            ...
                            <td style="text-align: center; padding-top: 0px; padding-bottom: 0px; color: lightgrey; font-size: 8pt; white-space: nowrap;">
                                <?php
                                $falta_count = count(array_filter($faltas_b, function($f) use ($jogador) {
                                    return $f['id_participante'] == $jogador['id'];
                                }));
                                echo implode(' | ', array_fill(0, min($falta_count, 5), 'X')) . str_repeat(' | ', 5 - min($falta_count, 5));
                                ?>
                            ...
                            <td style="text-align: center; padding-left: 0px; padding-right: 0px; padding-top: 0px; padding-bottom: 0px;">
                                <?php echo array_filter($cartoes_b, function($c) use ($jogador) {
                                    return $c['id_participante'] == $jogador['id'] && $c['tipo_evento'] == 'Cartão Amarelo';
                                }) ? '<span class="font-menor">X</span>' : '<span class="font-menor"> | </span>'; ?>
                            </td>
                            <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;">
                    ...
                    <?php endfor; ?>
                    <tr>
                        <td style="padding-top: 0px; padding-bottom: 0px;" colspan="3"><b>Técnico:</b> <span contenteditable="true"><?= htmlspecialchars($partida['tecnico_b'] ?? '') ?></span></td>
                        <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;"></td>
                        <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;"></td>
                        <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;"></td>
                        <td style="text-align: center; padding-top: 0px; padding-bottom: 0px;"></td>

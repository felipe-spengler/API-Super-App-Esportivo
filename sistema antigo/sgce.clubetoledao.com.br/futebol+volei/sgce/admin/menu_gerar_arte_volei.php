...
<div class="row">
    <?php
    $categorias_todas = [
        'craque' => ['titulo' => 'Craque do Jogo', 'icone' => 'fas fa-star', 'cor' => 'primary'],
        'levantadora' => ['titulo' => 'Melhor Levantadora', 'icone' => 'fas fa-hand-sparkles', 'cor' => 'primary'],
        'libero' => ['titulo' => 'Melhor Líbero', 'icone' => 'fas fa-shield-alt', 'cor' => 'danger'],
        'oposta' => ['titulo' => 'Melhor Oposta', 'icone' => 'fas fa-bullseye', 'cor' => 'warning'],
        'ponteira' => ['titulo' => 'Melhor Ponteira', 'icone' => 'fas fa-running', 'cor' => 'success'],
        'central' => ['titulo' => 'Melhor Central', 'icone' => 'fas fa-tower-broadcast', 'cor' => 'info'],
        'pontuador' => ['titulo' => 'Maior Pontuador Geral', 'icone' => 'fas fa-trophy', 'cor' => 'dark'],
        'saque' => ['titulo' => 'Maior Pontuadora em Saque', 'icone' => 'fas fa-volleyball-ball', 'cor' => 'secondary'],
        'bloqueio' => ['titulo' => 'Melhor Bloqueio', 'icone' => 'fas fa-hand-paper', 'cor' => 'info'],
        'estreante' => ['titulo' => 'Melhor Estreante', 'icone' => 'fas fa-user-plus', 'cor' => 'success'],
    ];
...
// Garantir que as variáveis estão definidas para este modal
    ...
    ?>
    <div class="modal fade" id="modalGerarArteCraqueDedicado" tabindex="-1" aria-labelledby="modalGerarArteCraqueDedicadoLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalGerarArteCraqueDedicadoLabel">Gerar Arte - Craque do Jogo (MVP)</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <form id="formGerarArte_craque" action="gerar_arte_craque_volei.php" method="POST" target="_blank" enctype="multipart/form-data">
                    <input type="hidden" name="id_partida"       id="id_partida_craque">
                    <input type="hidden" name="id_equipe_a"      id="id_equipe_a_craque">
                    <input type="hidden" name="id_equipe_b"      id="id_equipe_b_craque">
                    <input type="hidden" name="nome_campeonato"  id="nome_campeonato_craque">
                    <input type="hidden" name="rodada"           id="rodada_craque">
                    <input type="hidden" name="placar"           id="placar_craque">
                    ...
                    <div class="modal-body">
                        ...
                        <!-- Seleção de Partida -->
                        <div class="mb-3">
                            <label for="partida_craque" class="form-label">Selecionar Partida</label>
                            <select class="form-control" id="partida_craque" name="id_partida_select" required>
                                <option value="">Selecione uma partida</option>
                            </select>
                        </div>

                        <!-- MVP (só aparece depois de escolher a partida) -->
                        <input type="hidden" name="id_jogador" id="id_jogador_craque">

                        <div class="mb-3">
                            <label class="form-label">Craque do Jogo (MVP)</label>
                            <p class="form-control-static fw-bold text-primary" id="nome_jogador_craque_craque">
                                <span class="text-muted">Selecione uma partida acima acima.</span>
                            </p>
                        </div>

                        <input type="hidden" id="foto_selecionada_craque" name="foto_selecionada" value="">

                        <!-- Fotos do MVP -->
                        <div id="fotos_jogador_craque" class="mt-3"></div>

                        <!-- Adicionar nova foto -->
                        <div class="mb-3">
                            <button type="button" class="btn btn-primary btn-sm" id="btnAdicionarFoto_craque">
                                <i class="fas fa-plus me-1"></i> Adicionar Foto
                            </button>
                            <input type="file" id="inputNovaFoto_craque" name="nova_foto" accept="image/*" style="display: none;">
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-success" id="btnGerarArte_craque">
                            <i class="fas fa-image me-1"></i> Gerar Arte
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div class="modal fade" id="<?= $modal_id ?>" tabindex="-1" aria-labelledby="<?= $modal_id ?>Label" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form id="formGerarArte_<?= $categoria ?>" action="gerar_arte_volei.php" method="POST" target="_blank" enctype="multipart/form-data">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const categoriasSemPartida = ['levantadora', 'libero', 'oposta', 'ponteira', 'central', 'pontuador', 'saque', 'bloqueio', 'estreante'];        // Função para preencher select de partidas
...
<?php foreach ($categorias as $categoria => $info): ?>
                ...
                btnGerarArte_<?= $categoria ?>?.addEventListener('click', (e) => {
                    ...
                    fetch('/sgce/admin/salvar_jogador_categoria_volei.php', {
                        method: 'POST',
                        headers: {

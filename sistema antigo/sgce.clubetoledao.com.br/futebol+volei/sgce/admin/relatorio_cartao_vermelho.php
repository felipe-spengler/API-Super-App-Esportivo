...
// 2. Busca os eventos de cartão vermelho para o campeonato, agrupados por jogador e equipe
$sql_cartoes_vermelhos = "
    SELECT 
        par.id as id_participante,
        par.nome_completo as nome_participante,
        eq.nome as nome_equipe,
        eq.id as id_equipe,
        COUNT(se.id) as total_cartoes_vermelhos
    ...
    WHERE p.id_campeonato = ? AND se.tipo_evento = 'Cartão Vermelho'
    GROUP BY par.id, eq.id
    ORDER BY total_cartoes_vermelhos DESC, par.nome_completo ASC
";
$stmt_cartoes_vermelhos = $pdo->prepare($sql_cartoes_vermelhos);
$stmt_cartoes_vermelhos->execute([$id_categoria]);
$cartoes_vermelhos = $stmt_cartoes_vermelhos->fetchAll(PDO::FETCH_ASSOC);
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Cartões Vermelhos</li>
    </ol>
</nav>

<main class="container py-5">
    <div class="card">
        <div class="card-header"><i class="fas fa-id-card text-danger me-2"></i>Cartões Vermelhos do Campeonato</div>
        <div class="card-body">
            <?php if (count($cartoes_vermelhos) > 0): ?>
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            foreach ($cartoes_vermelhos as $cartao): 
                            ...
                            ?>
                                <tr class="caption-event <?= $row_class ?>">
                                    ...
                                    <td><?= htmlspecialchars($cartao['total_cartoes_vermelhos']) ?></td>
                                </tr>
                                <tr class="body-event d-none">
                                    <td colspan="3">
                                        <?php
                                        // Busca as partidas onde este jogador recebeu cartões vermelhos
                                        $sql_partidas = "
                                            SELECT DISTINCT
                                                p.id as id_partida,
                                            ...
                                            AND p.id_campeonato = ? 
                                            AND se.tipo_evento = 'Cartão Vermelho'
                                            GROUP BY p.id
                                        ...
                                        ?>
                                            ...
                                            <table class="table inner-table">
                                                <?php
                                                // Busca os cartões vermelhos detalhados para a partida
                                                $sql_cartoes = "
                                                    SELECT 
                                                    ...
                                                    LEFT JOIN participantes par ON se.id_participante = par.id
                                                    LEFT JOIN equipes eq ON se.id_equipe = eq.id
                                                    WHERE se.id_partida = ? AND se.tipo_evento = 'Cartão Vermelho'
                                                    GROUP BY par.id, eq.id
                                                ...
                                                <?php if (!empty($cartoes_equipe_a)): ?>
                                                    <thead>
                                                        <tr>
                                                            <th colspan="4"><div class="equipe-a-header w-100">Cartões Vermelhos de <?= htmlspecialchars($partida['nome_equipe_a']) ?></div></th>
                                                        </tr>
                                                        <tr>
                                                            <th>Número</th>
                                                            <th>Jogador</th>
                                                ...
                                                <?php if (!empty($cartoes_equipe_b)): ?>
                                                    <thead>
                                                        <tr>
                                                            <th colspan="4"><div class="equipe-b-header w-100">Cartões Vermelhos de <?= htmlspecialchars($partida['nome_equipe_b']) ?></div></th>
                                                        </tr>
                                                        <tr>
                                                            <th>Número</th>
                                                            <th>Jogador</th>
                                            ...
                                            <?php if (empty($cartoes_equipe_a) && empty($cartoes_equipe_b)): ?>
                                                <div class="alert alert-info text-center">Nenhum cartão vermelho registrado para esta partida.</div>
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                                        <?php else: ?>
                                            <div class="alert alert-info text-center">Nenhum cartão vermelho registrado para este jogador.</div>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <?php else: ?>
                <div class="alert alert-secondary text-center">Nenhum cartão vermelho foi registrado para este campeonato.</div>
            <?php endif; ?>
        </div>
    </div>
</main>

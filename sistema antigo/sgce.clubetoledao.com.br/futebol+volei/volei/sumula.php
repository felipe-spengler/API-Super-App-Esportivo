...
// Adapte o getPeriodoEMinuto para Vôlei
function getPeriodoAtivo($pdo, $id_partida) {
    // Busca apenas o último Set iniciado e não finalizado
    $stmt_current = $pdo->prepare("SELECT periodo FROM sumulas_periodos WHERE id_partida = ? AND hora_fim IS NULL AND periodo NOT LIKE 'Pausa %' ORDER BY id DESC LIMIT 1");
    $stmt_current->execute([$id_partida]);
    return $stmt_current->fetchColumn() ?: null;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['registrar_tempo'])) {
    ...
    try {
        ...
        $periodsOrder = ['1º Set', '2º Set', '3º Set', '4º Set', '5º Set']; // Geralmente 5 sets

        if ($acao === 'atualizar_tempos') {
            $response = $stmt_periodos = $pdo->prepare("SELECT * FROM sumulas_periodos WHERE id_partida = ? ORDER BY id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_evento'])) {
    ...
    $current_period = getPeriodoAtivo($pdo, $id_partida);
...
$periodsOrder = ['1º Set', '2º Set', '3º Set', '4º Set', '5º Set']; // VÔLEI
    $next_period = null;
if (!$current_period) {
    if (!$last_finalized) {
        $next_period = $periodsOrder[0]; // '1º Set'
    } else {
        $last_index = array_search($last_finalized, $periodsOrder);
        // Garante que o próximo set não seja depois do 5º Set
...
require_once '../sgce/includes/header.php'; // Header público
?>
...
<div class="card mb-4">
    <div class="card-header"><i class="fas fa-stopwatch me-1"></i> Relógio dos Sets (Tempo de Jogo Total)</div>
    <div class="card-body">
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th>Set</th> <th>Cronômetro (Duração Efetiva)</th>
                    <th>Time-Outs (Início - Fim, Duração, Equipe)</th>
                </tr>
            </thead>
            <tbody>

                <?php
                // Gerar as 5 linhas da tabela para os Sets de Vôlei
                $sets_volei = ['1º Set', '2º Set', '3º Set', '4º Set', '5º Set'];
                foreach ($sets_volei as $set_nome):
                    $set_id = strtolower(str_replace(' ', '_', $set_nome)); // Ex: '1º_set'
                    $pausa_periodo = 'Pausa ' . $set_nome;
                    ?>

                    <tr>
                        <td><?= htmlspecialchars($set_nome) ?></td>
                        <td><span id="chrono_<?= $set_id ?>">00:00:00</span></td>
                        <td id="pausas_<?= $set_id ?>">
                            <?php
                            // Filtrar Time-Outs para o Set atual
                            $pausas_set = array_filter($timeouts, function ($t) use ($pausa_periodo) {
                                return $t['periodo'] === $pausa_periodo;
                            });

                            echo implode(', ', array_map(function ($t) use ($partida) {
                                        // CÁLCULO DA DURAÇÃO DE UM TIME-OUT (A PAUSA)
                                        $start_time = strtotime($t['hora_inicio']);
                                        $end_time = $t['hora_fim'] ? strtotime($t['hora_fim']) : time();
                                        $duration_sec = $end_time - $start_time;

                                        $str_duration = gmdate("i:s", $duration_sec); // Formato MM:SS
                                        ...
                                        $str .= " ($str_duration, $equipe)";
                                        return htmlspecialchars($str);
                                    }, $pausas_set)) ?: 'Nenhum Time-Out';
                            ?>
                        </td>
                    </tr>
                    ...
                    <?php else: ?>
                        <?php foreach ($eventos as $evento): ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                    // VÔLEI (ALTERADO) - Adapte conforme suas estatísticas
                                    switch ($evento['tipo_evento']) {
                                        case 'Ponto de Ataque':
                                            echo '<span class="badge bg-success"><i class="fas fa-fist-raised me-1"></i> Ponto (Ataque)</span>';
                                            break;
                                        case 'Ponto de Bloqueio':
                                            echo '<span class="badge bg-info"><i class="fas fa-shield-alt me-1"></i> Ponto (Bloqueio)</span>';
                                            break;
                                        case 'Ponto de Saque':
                                            echo '<span class="badge bg-warning text-dark"><i class="fas fa-hand-point-up me-1"></i> Ponto (Saque)</span>';
                                            break;
                                        case 'Substituição':
                                            echo '<span class="badge bg-secondary"><i class="fas fa-exchange-alt me-1"></i> Substituição</span>';
                                            break;
                                        // Mantenha Cartões Amarelo/Vermelho se aplicáveis
                                        case 'Cartão Amarelo':
...
<script>
    ...
    const periodsOrder = ['1º Set', '2º Set', '3º Set', '4º Set', '5º Set']; // VÔLEI (ALTERADO)
    let periodsData = [
...
<?php endforeach; ?>
    ...
    let periodPauses = pauses.filter(p => p.periodo === `Pausa ${period}`); // Filtra pelo Set atual
    ...
    // No Vôlei, o Time-Out é fixo (geralmente 30s) mas a duração aqui é calculada
    const duration_ms = p.end ? p.end - p.start : Date.now() - p.start;
    const duration_sec = Math.floor(duration_ms / 1000);
    const duration_str = `${Math.floor(duration_sec / 60).toString().padStart(2, '0')}:${(duration_sec % 60).toString().padStart(2, '0')}`;
    ...
    return `${startTime} - ${endTime} (${duration_str}, ${equipe})`;
    }).join(', ');
    pauseElement.innerHTML = pauseStr || 'Nenhum Time-Out';
    }
    }
    ...
    // No Vôlei, Set é um período simples, não precisa de filtro por equipe (como Pênalti no Futebol)
    const periodData = periodsData.filter(p => p.periodo === period);
    ...
    let totalPausedMs = 0;
    // Pausas em Vôlei são "Pausa 1º Set", "Pausa 2º Set", etc.
    const periodPauses = pauses.filter(p => p.periodo === `Pausa ${period}`);
...
<?php
require_once '../sgce/includes/footer.php'; // Footer público ?>
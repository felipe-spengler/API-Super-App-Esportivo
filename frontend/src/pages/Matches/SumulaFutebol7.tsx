...
// FoulDots também fora do componente — indicador de faltas acumuladas
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    { t: 'away' as const, team: matchData.away_team, score: awayScore, foulCount: fouls.away, color: 'emerald' }
                        ...
                        <div key={t} className={`relative bg-[#111827]/40 rounded-[2.5rem] p-4 border border-white/5 flex flex-col items-center shadow-xl transition-all duration-500 ${confirmationEffect === t ? 'ring-2 ring-emerald-500/50 scale-[1.02]' : ''}`}>
                            ...
                            {/* Foul counter + free kick indicator */}
                        ...
                        <div className="p-4 overflow-y-auto flex-1">
                            ...
                            <div className="space-y-2">
                                {(selectedTeam === 'home' ? rosters.home : rosters.away).sort((a: any, b: any) => parseInt(a.pivot?.number || a.number || 0) - parseInt(b.pivot?.number || b.number || 0)).map((p: any) => (
                                    <button key={p.id} onClick={() => confirmEvent(p)} className="w-full group px-6 py-4 bg-[#1a2234]/60 hover:bg-blue-600/20 border border-white/5 hover:border-blue-500/50 rounded-2xl transition-all flex items-center justify-between">
                                        <div className="flex items-center gap-5">
                                            <div className="w-10 h-10 shrink-0 rounded-xl bg-black/40 flex items-center justify-center text-lg font-black text-white group-hover:text-blue-400 tabular-nums">{p.pivot?.number || p.number || '#'}</div>
                                            <div>
                                        ...
                                        <Icon icon="heroicons-solid:chevron-right" className="w-5 h-5 text-gray-700 group-hover:text-blue-400" />
                                    </button>
                                ))}

...
export const PrintHandball = ({ match, rosters, events }: { match: any, rosters: any, events: any[] }) => {
    const TeamBlock = ({ team, players }: { team: any, players: any[] }) => {
        ...
        const rows = Array.from({ length: 16 }, (_, i) => {
            ...
            return {
                ...
                yellow: pEvents.some(e => e.type === 'yellow_card'),
                red: pEvents.some(e => e.type === 'red_card'),
                blue: pEvents.some(e => e.type === 'blue_card'),
                suspensions: pEvents.filter(e => e.type === 'suspension' || e.type === '2min').length,
            ...
            <div className="mb-6 break-inside-avoid">
                ...
                <table className="w-full text-[10px] border-collapse border border-black">
                    <thead className="bg-gray-200 text-center font-bold">
                        <tr>
                            <th className="border border-black w-6" rowSpan={2}>Nº</th>
                            <th className="border border-black text-left px-1" rowSpan={2}>Nome / Atleta</th>
                            <th className="border border-black w-8" rowSpan={2}>Cam.</th>
                            <th className="border border-black" colSpan={2}>Participação</th>
                            <th className="border border-black w-6" rowSpan={2}>G</th>
                            <th className="border border-black w-6" rowSpan={2}>A</th>
                            <th className="border border-black" colSpan={2}>2'</th>
                            <th className="border border-black w-6" rowSpan={2}>V</th>
                            <th className="border border-black w-6" rowSpan={2}>D</th>
                        </tr>
                        <tr className="text-[8px]">
                            <th className="border border-black w-6">1º</th>
                            <th className="border border-black w-6">2º</th>
                            <th className="border border-black w-6">1ª</th>
                            <th className="border border-black w-6">2ª</th>
                        </tr>
                    </thead>
                    <tbody>
                        {rows.map((row) => (
                            <tr key={row.idx} className="h-6">
                                ...
                                <td className="border border-black text-center font-bold">{row.goals || ''}</td>
                                <td className="border border-black text-center font-bold">{row.yellow ? 'X' : ''}</td>
                                <td className="border border-black text-center font-bold">{row.suspensions >= 1 ? 'X' : ''}</td>
                                <td className="border border-black text-center font-bold">{row.suspensions >= 2 ? 'X' : ''}</td>
                                <td className="border border-black text-center font-bold">{row.red ? 'X' : ''}</td>
                                <td className="border border-black text-center font-bold">{row.blue ? 'X' : ''}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td className="border border-black p-2 font-bold text-center w-[20%] bg-gray-50">
                            ...
                            <div className="text-[9px] font-normal uppercase text-blue-600 mt-1">Gols Finais</div>
                        </td>
            ...
            <div className="grid grid-cols-2 gap-4">
                <table className="w-full text-[10px] border-collapse border border-black">
                    <tbody>
                        ...
                        <tr><td className="border border-black p-1"><b>Arbitragem:</b> <EditableSpan text={match.match_details?.arbitration?.referee} /></td></tr>
                    </tbody>
                </table>
                <table className="w-full text-[9px] border-collapse border border-black text-center">
                    <thead className="bg-gray-100 font-bold">
                        <tr><th className="border border-black">Tempo</th><th className="border border-black">Placar</th><th className="border border-black">Início</th><th className="border border-black">Fim</th></tr>
                    </thead>
                    <tbody>
                        {[1, 2, 3].map(n => {
                            const label = n <= 2 ? `${n}º Tempo` : 'Extra';

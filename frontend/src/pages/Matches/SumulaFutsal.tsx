...
function ActionGrid({ homeTeamName, awayTeamName, isRunning, isPenaltyPeriod, currentPeriod, onOpenEvent, onOpenCardFlow, onOpenMvpFlow, onTimeout }: ActionGridProps) {
    const isTimeoutDisabled = ['Intervalo', 'Fim de Tempo Normal', 'Prorrogação', 'Pênaltis'].includes(currentPeriod);
        ...
        <div className={`bg-gradient-to-b ${gradient} border ${border} rounded-3xl p-3 space-y-2`}>
            ...
            <div className="grid grid-cols-2 gap-1.5">
                ...
                <ActionBtn 
                    ...
                    disabled={!isRunning || isTimeoutDisabled}
...
export function SumulaFutsal() {
    ...
    const fetchMatchDetails = async (isInitial = false) => {
        try {
            ...
            if (data.match) {
                ...
                // Re-calcula placar de pênaltis e faltas da rodada atual
    ...
    const handlePeriodChange = () => {
        ...
        if (currentPeriod === '1º Tempo') {
            ...
            setIsRunning(true);
            setTime(0);
            setFouls({ home: 0, away: 0 }); // Zera as faltas coletivas para o 2º tempo
            ...
            timerRef.current = { ...timerRef.current, currentPeriod: newPeriod, isRunning: false };
            registerSystemEvent('period_end', 'Fim do Tempo Normal. Prorrogação ou Pênaltis à vista?');
    ...
    const registerSimpleEvent = async (team: 'home' | 'away', type: 'timeout') => {
        ...
        // Validação de limite: máximo 1 timeout por período no Futsal
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm">
            <div className="bg-[#111827] w-full max-w-md sm:rounded-3xl rounded-t-3xl border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
                ...
                <div className="overflow-y-auto p-4 flex-1">
                    ...
                    {eventType === 'foul' && (
                        <button onClick={() => onSelect(null)} className="w-full py-5 mb-4 rounded-3xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-3 bg-white/5 text-gray-400 border border-white/10 hover:bg-white/10 hover:text-white">
                ...
                <div className="flex flex-col">
                    <div className="flex items-center gap-1.5 flex-wrap">
                        <span className={`font-bold text-sm ${isOwnGoal && ev.type === 'goal' ? 'text-red-300' : 'text-gray-100'}`}>
                            {foulAlertLabel || eventLabels[ev.type] || ev.type}
                        </span>
